...
public class scriptLanceTejo : MonoBehaviour
{
    ...
    ///
    void lanzarTejo(){
        if( tejoALanzar.Equals(nombreTejo)){
        ...
        } else if ( tejoALanzar.Equals(nombreTejoCPU)){
                Debug.Log ("**** en scriptLanceTejo.lanzando tejocpu dis magnitude "+dis.magnitude+" maxVel "+maxVel+" time "+System.DateTime.Now);
                BoxCollider2D boxColliderTejoCPU = GameObject.Find (nombreTejoCPU).GetComponent<BoxCollider2D>();
                boxColliderTejoCPU.enabled = true;
                Animator animCPU = GameObject.Find (nombreTejoCPU).GetComponent<Animator>();
                AnimatorStateInfo currentBaseState = animCPU.GetCurrentAnimatorStateInfo(0);
                if( currentBaseState.nameHash != stateTejoEnAire){
                    animCPU.SetBool(BOOL_TEJO_EN_AIRE,true);
                }
                rbTejo = GameObject.Find (nombreTejoCPU).GetComponent<Rigidbody2D>();
                rbTejo.bodyType = RigidbodyType2D.Dynamic;
                rbTejo.velocity = -dis.normalized * maxVel;
            }
           
    }
    public void copiarValores(Vector3 elDis, float elMaxVel, string tejoParaLanzar ){
        dis = elDis;maxVel = elMaxVel;tejoALanzar = tejoParaLanzar;
        Debug.Log ("**** en scriptLanceTejo.copiando dis magnitude "+dis.magnitude+" dis vector "+elDis+" time "+System.DateTime.Now+" tejoParaLanzar "+tejoParaLanzar);
    }
