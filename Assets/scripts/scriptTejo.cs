...
public class scriptTejo : MonoBehaviour
{
    ...
    public static string ANIMACION_GUIA_MECANICA = "guiaMecanica";
    public static string ANIMACION_POINTER_CIRCLE = "circlePointer";
    ...
    public static string BOOL_START_ANIMAR_CIRCLE = "startCircle";
    public static string BOOL_FIN_ANIMAR_CIRCLE = "finCircle";
    string TEXT_SCORE_CPU = "textScoreCPU";
    string TEXT_SCORE_PLAYER = "textScorePlayer";
    string TEXT_SCORE_TO_WIN = "textScoreToWin";
    string TEXT_DEBUG = "TextDebug";
    ...
    void Start()
    {
        ...
        Animator animCircle = GameObject.Find (ANIMACION_POINTER_CIRCLE).GetComponent<Animator>();
        animCircle.SetBool(BOOL_START_ANIMAR_CIRCLE,true);
        animCircle.SetBool(BOOL_FIN_ANIMAR_CIRCLE,false);
        Debug.Log ("**** en start scriptTejo ");
    ...
    void Update()
    {
        ...
        Debug.Log ("**** update de scriptTejo camb 1");
        if (Input.touchCount > 0)
        {
            Touch touch = Input.GetTouch(0);
            Debug.Log ("**** touch detectado  ");
            if(  GameObject.Find (TEXT_DEBUG) != null ){
                strTextDebug +=" en update "+Input.mousePosition;
                GameObject.Find (TEXT_DEBUG).gameObject.GetComponent<Text>().text = strTextDebug;
            } 
            // Move the cube if the screen has the finger moving.
            Vector2 pos = touch.position;
            Debug.Log ("**** touch detectado  "+ ((pos.x - width) / width));
            //pos.x = (pos.x - width) / width;
            ////posiblemente esta linea hacer fallar la funcionalidad en android
            ///GameObject.Find (NOMBRE_TEJO_GUIA).GetComponent<Transform>().position = pos;
        }
        
        if(strTextDebug.Length > LIMITE_STR_DEBUG ){
            strTextDebug = "";
        }
        if(  GameObject.Find (TEXT_DEBUG) != null ){
            Vector3 posTrans = GetComponent<Transform>().position;
            string name = GetComponent<Transform>().name;
            if( GetComponent<Transform>().position != posAnterior){
                strTextDebug +=" en update ["+name+"]"+posTrans;
                GameObject.Find (TEXT_DEBUG).gameObject.GetComponent<Text>().text = strTextDebug;
                posAnterior = GetComponent<Transform>().position;
            }
        } 
    }
    void OnMouseDown(){
        Debug.Log ("**** onmousedown scriptTejo  ");
        if(  GameObject.Find (TEXT_DEBUG) != null ){
            strTextDebug +=  " en mouse down "+Input.mousePosition;
            GameObject.Find (TEXT_DEBUG).gameObject.GetComponent<Text>().text = strTextDebug;
        } 
    ...
    void OnMouseDrag(){
        Debug.Log ("**** en mouse drag ");
        if(  GameObject.Find (TEXT_DEBUG) != null ){
            strTextDebug +=  " en drag "+Input.mousePosition;
            GameObject.Find (TEXT_DEBUG).gameObject.GetComponent<Text>().text = strTextDebug;
        } 
        if(!canDrag){
            return;
        ...
        Animator animCircle = GameObject.Find (ANIMACION_POINTER_CIRCLE).GetComponent<Animator>();
        animCircle.SetBool(BOOL_FIN_ANIMAR_CIRCLE,true);
        animCircle.SetBool(BOOL_START_ANIMAR_CIRCLE,false);
        ...
        dis = pos - pivot.position;////pivot es el tejo pequenio que sirve de guia de centro para medir la posicion del touch  cuan lejos esta del centro
        dis.z = 0;///estaba en 0
        ...
        Button buttonHighScoresTeams = GameObject.Find (Collision.NAME_CANVAS).GetComponent<Transform>().Find(Collision.BUTTON_HIGHSCORES_TEAMS).GetComponent<Button>();
        buttonHighScoresTeams.gameObject.SetActive(false);
        ...
        //// el tejo grande q esta sobre el tejo central de mueve segun la distancia del toque del mouse o dedo en el touch 
    ...
    void OnMouseUp(){
        Debug.Log ("**** en moouse up ");
        if(!canDrag)
            return;
        float dist = Vector3.Distance(GetComponent<Transform>().position, POSICION_INICIAL_TEJO_GUIA);    
        Debug.Log ("**** distance antes de ocultar  "+dist);
        /// hubo un touch por fuera del tejo arrastrable    
        if(  GetComponent<Transform>().position == POSICION_INICIAL_TEJO_GUIA ){
            return;
        }
        canDrag = false;
        ...
        //// se desaparece aki, no es q cambie de posicion ujummm
        Debug.Log ("**** ocultando tejo ");
