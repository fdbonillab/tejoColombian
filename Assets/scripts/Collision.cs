...
public class Collision : MonoBehaviour
{
    ...
    string BUTTON_TEAM_DEPARTAMENT = "ButtonDepartamento";
    string BUTTON_TEAM_FUTBOL = "ButtonFutbol";
    ...
    List<Departamento> lstTeamsFutbol;
    ...
    */
    ...
    void Update()
    {
        ...
        if (Input.GetKeyDown(KeyCode.Escape)){
            ...
            agregarTeamEnPreferencesAResultado(ultimoResultado);
    ...
    private void setBanderaEquipo(){
        ...
        if( lstTeamsFutbol == null ){
            llenarListaTeamsFutbol();
        }
         string teamEnPreferenceTeamFutbol = PlayerPrefs.GetString(scriptSelectorEquipoFutbol.NAME_PREF_TEAM_FUTBOL);
         string teamEnPreferenceDepartamento = PlayerPrefs.GetString(scriptSelectorEquipoFutbol.NAME_PREF_TEAM_DEPARTAMENT);
         Debug.Log ("**** teamEnPreferenceTeamFutbol en setBanderaEquipo "+teamEnPreferenceTeamFutbol);
         Debug.Log ("**** teamEnPreferenceDepartamento en setBanderaEquipo "+teamEnPreferenceDepartamento);
         if( (teamEnPreferenceDepartamento != null && !teamEnPreferenceDepartamento.Equals("")) ){
              Button buttonEquipoDepartament = GameObject.Find (NAME_CANVAS).GetComponent<Transform>().Find(BUTTON_TEAM_DEPARTAMENT).GetComponent<Button>();
              string nameImageFoundedDepartament = getImageTeamByName(teamEnPreferenceDepartamento);
              Debug.Log ("**** nameImageFoundedDepartament en setBanderaEquipo "+nameImageFoundedDepartament);
              Sprite _sprite =  atlasDepartamentos.GetSprite(nameImageFoundedDepartament);
              if( _sprite != null ){
                 Debug.Log ("****  setBanderaEquipo sprite != null ");
                 buttonEquipoDepartament.gameObject.transform.GetChild(1).GetComponent<Image>().sprite = _sprite;
              } 
              buttonEquipoDepartament.GetComponent<Button>().GetComponentInChildren<Text>().text = teamEnPreferenceDepartamento;
         }
         if( (teamEnPreferenceTeamFutbol != null && !teamEnPreferenceTeamFutbol.Equals("")) ){
              Button buttonEquipoFutbol = GameObject.Find (NAME_CANVAS).GetComponent<Transform>().Find(BUTTON_TEAM_FUTBOL).GetComponent<Button>();
              string nameImageFoundedFutbol = getImageTeamByName(teamEnPreferenceTeamFutbol);
              Debug.Log ("**** nameImageFoundedFutbol en setBanderaEquipo "+nameImageFoundedFutbol);
              Sprite _spriteFutbol =  atlasTeamsFutbol.GetSprite(nameImageFoundedFutbol);
              if(_spriteFutbol != null  ){
                 Debug.Log ("**** setBanderaEquipo sprite == null, spritefutbol == null  "+(_spriteFutbol == null ));
                 buttonEquipoFutbol.gameObject.transform.GetChild(1).GetComponent<Image>().sprite = _spriteFutbol;
              }
              buttonEquipoFutbol.GetComponent<Button>().GetComponentInChildren<Text>().text = teamEnPreferenceTeamFutbol;
         }
    }
    private  string getImageTeamByName(string nameTeam ){
        ...
        foreach(Departamento elDepartamento in lstTeamsFutbol ){
             Debug.Log ("**** elDepartamento.getNombre() "+elDepartamento.getNombre()+" "+nameTeam);
            if( elDepartamento.getNombre().Equals(nameTeam)){
                return elDepartamento.getNombreBandera();
            }
        }
    ...
    ///// orden del otro lado millonarios nacional santafe america tolimn
     private void llenarListaTeamsFutbol(){
        Departamento elDepartamento = new Departamento( scriptSelectorEquipoFutbol.NAME_MILLONARIOS,1,scriptSelectorEquipoFutbol.NAME_IMG_ESCUDO_MILLONARIOS);
        lstTeamsFutbol = new List<Departamento>();
        lstTeamsFutbol.Add(elDepartamento);
        elDepartamento = new Departamento( scriptSelectorEquipoFutbol.NAME_NACIONAL,2,scriptSelectorEquipoFutbol.NAME_IMG_ESCUDO_NACIONAL );
        lstTeamsFutbol.Add(elDepartamento);
        elDepartamento = new Departamento( scriptSelectorEquipoFutbol.NAME_SANTAFE,3,scriptSelectorEquipoFutbol.NAME_IMG_ESCUDO_SANTAFE );
        lstTeamsFutbol.Add(elDepartamento);
        elDepartamento = new Departamento( scriptSelectorEquipoFutbol.NAME_AMERICA,4,scriptSelectorEquipoFutbol.NAME_IMG_ESCUDO_AMERICA );
        lstTeamsFutbol.Add(elDepartamento);
        elDepartamento = new Departamento( scriptSelectorEquipoFutbol.NAME_TOLIMA,5,scriptSelectorEquipoFutbol.NAME_IMG_ESCUDO_TOLIMA );
        lstTeamsFutbol.Add(elDepartamento);
    }
    ...
    private void sumarScoreATeam(Resultado elResultado){
        ...
        ///// suponiendo q algun dia se pida tanta precision el puntaje anterior solo se confirma si se se revisa el success del manager score
    ...
    bool validarGanoPlayer(){
        ...
        if( scorePlayer != scoreCPU && scorePlayer > scoreWin && turnosPlayer == turnosCPU && turno == TURNO_PLAYER && !finDeJuego){
                ...
                agregarTeamEnPreferencesAResultado(toResultado);
                string json = JsonUtility.ToJson(toResultado);
                managerScoreJson.salvarScore(json);
                return true;
        } else {
    ...
    private void agregarTeamEnPreferencesAResultado(  Resultado _resultado ){
        //teamEnPreference = null;
        //PlayerPrefs.SetString( NAME_PREF_TEAM, null );
        Team _team = new Team();
        _resultado.team = _team;
        string elNameTeam = PlayerPrefs.GetString( scriptSelectorEquipo.NAME_PREF_TEAM );
        _team.nameTeam = elNameTeam;
    }
