...
public class ManagerUser : App42CallBack
{
    ...
    public static void crearUsuario(){
          ...
          guardarUnResultadoConNuevoUser(userName);
          userService.CreateUser(userName, pwd, emailId,new ManagerUser());  
    }
    private static void guardarUnResultadoConNuevoUser( string nuevoUserName ){
        int turnosFicti =1;
        Debug.Log ("**** guardarunresultado con nuevo user" );
        ManagerScoresJson managerScoreJson = new ManagerScoresJson();
        bool esPC =  (Application.platform != RuntimePlatform.Android);;
        int bestScoreEnPreference = PlayerPrefs.GetInt(Collision.NAME_PREF_BEST_SCORE);
        Resultado toResultado = new Resultado(nuevoUserName,Collision.LEVEL_1, Collision.PUNTAJE_OBJETIVO_10, bestScoreEnPreference, turnosFicti, esPC );
