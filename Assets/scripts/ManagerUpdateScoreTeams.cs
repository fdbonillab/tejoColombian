...
public class ManagerUpdateScoreTeams : App42CallBack
{
    ...
    */
    ...
    public static String collectionNameHighScoresTeams = "highscoreTeams"; 
    ...
    public void salvarScore(string unJson){
     ...
     public void actualizarScore( string docId, string unJson ){
          Debug.Log ("**** actualizando score team hight en app42 appwarp storageService");
          if( storageService == null ){
             Start();
          }
          if(storageService != null ){
              //storageService.UpdateDocumentByDocId(dbName, collectionName, docId, jsonDoc, new UnityCallBack());   
              storageService.UpdateDocumentByDocId(dbName, ManagerScoresJson.collectionNameHighScoresTeams, docId,unJson, new ManagerUpdateScoreTeams());  
          } else {
              Debug.Log ("**** storageService es null ");
          }
     }
     public void salvarScoreTeams( string unJson ){
          Debug.Log ("**** salvando score by team hight en app42 appwarp storageService");
          if( storageService == null ){
             Start();
          }
          if(storageService != null ){
              storageService.InsertJSONDocument(dbName, collectionNameHighScoresTeams, unJson, new ManagerScoresJson());  
          } else {
              Debug.Log ("**** storageService es null ");
          }
    }
      public void OnSuccess(object response)  
            {  
                App42Log.Console("**** succes update teams app42 storageService");  
                Storage storage = (Storage) response;  
                IList<Storage.JSONDocument> jsonDocList = storage.GetJsonDocList();   
                for(int i=0;i <jsonDocList.Count;i++)  
                {     
                    App42Log.Console(" teams update objectId is " + jsonDocList[i].GetDocId());  
                    App42Log.Console(" teams update ejsonDoc is " + jsonDocList[i].GetJsonDoc());  
                    App42Log.Console(" teams update CreatedAt is : " + jsonDocList[i].GetCreatedAt());  
                    App42Log.Console(" teams update UpdatedAt is : " + jsonDocList[i].GetUpdatedAt());   
                }    
            }  
        
            public void OnException(Exception e)  
            {  
                App42Log.Console("**** failure fallo teams update app42  storageService Exception : " + e);  
            }  
