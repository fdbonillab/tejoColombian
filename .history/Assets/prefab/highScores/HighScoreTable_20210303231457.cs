...
public class HighScoreTable:MonoBehaviour
{
    ...
    private void CreateHighScoreEntryTransform(HighScoreEntry highScoreEntry, Transform container, List<Transform> listTransform){
        ...
        entryTransform.Find(NAME_IMG_TEAM_ESCUDO).GetComponent<RawImage>().texture = buscarImgTeam(highScoreEntry.nameDepartamento);
    ...
    private Sprite buscarImgTeam(string _nameTeam){
        Sprite _sprite = atlasEquiposFutbol.GetSprite(_nameTeam);  
        if( _sprite == null ){
            _sprite =  atlasDepartamentos.GetSprite(_nameTeam);  
        }
        return _sprite;
    }
