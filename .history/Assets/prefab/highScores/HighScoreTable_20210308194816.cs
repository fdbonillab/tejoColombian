...
public class HighScoreTable:MonoBehaviour
{
    ...
    void OnGUI(){
        ...
        subLstTeams = new ScoreTeams();
        Debug.Log ("**** subLstTeams == null "+( subLstTeams == null ));
      
        Debug.Log ("**** desde "+desde+" hasta "+hasta+" tablaMostrada "+tablaMostrada);
        Debug.Log ("**** pagina "+pagina);
       
        if( unScoreTeams != null && sizeListPrincipal > 0 ){
            if(  hasta >= sizeListPrincipal ){
                hasta = sizeListPrincipal-1;
            }
            Debug.Log ("**** desde "+desde+" hasta "+hasta);
            if( desde >= hasta ){
                desde = hasta - rowsPage;
            }
            Debug.Log ("**** desde "+desde+" hasta "+hasta+" unScoreTeams.getHighScoreTeamList() "+unScoreTeams.getHighScoreTeamList().Count);
            subLstTeams.setLstTeams(unScoreTeams.getHighScoreTeamList().GetRange( desde , rowsPage ));
        }
        //// error parti la lista total antes de ordenarla y entonces el ordanmiento general no quedo bien
        ///highScoreEntryList = ScoresTeams2HighScoreEntry(subLstTeams);
        if(  highScoreEntryListPrincipal == null &&  unScoreTeams != null ){
             highScoreEntryListPrincipal = ScoresTeams2HighScoreEntry(unScoreTeams);
        }
        highScoreEntryList = highScoreEntryListPrincipal.GetRange( desde , rowsPage );
        Debug.Log ("****  tablaMostrada "+tablaMostrada+"  highScoreEntryList.Count "+highScoreEntryList.Count+" subLstTeams.list.Count "+subLstTeams.getHighScoreTeamList().Count );
        highScoreEntryTransformList = new List<Transform>();
        if(  !tablaMostrada && highScoreEntryList.Count > 0 ){
            Debug.Log ("**** mostrando lista ");
            ...
            foreach (Transform child in entryContainer) {
                Debug.Log ("**** name child  "+child.gameObject.name);
    ...
    public void nextPage(){
        pagina ++;
        //// chamnbonada pagina maxima = 1 , pero de momento no es grave
        int paginaMaxima = 1;//(unScoreTeams.getHighScoreTeamList().Count)/rowsPage;
        if( pagina > paginaMaxima){
            pagina = paginaMaxima;
        }
        //// para forzar q recargue la pagina
        tablaMostrada = false;
        Debug.Log ("**** nextpage pagina "+pagina+" paginaMaxima "+paginaMaxima);
    }
    ...
    private void CreateHighScoreEntryTransform(HighScoreEntry highScoreEntry, Transform container, List<Transform> listTransform){
        ...
        Debug.Log ("**** rankString  "+rankString+" nameTeam "+highScoreEntry.nameDepartamento);
        int indiceEnPrincipal = rank;//getPositioninListPrincipal( highScoreEntry.nameDepartamento );
        Debug.Log ("**** indiceEnPrincipal  "+indiceEnPrincipal );
        Debug.Log ("**** pagina para rank  "+pagina );
        if( pagina >= 1){
            rank +=16;
        }
        entryTransform.Find(NAME_TEXT_POSICION).GetComponent<Text>().text = ""+rank;//rankString;
    ...
    private int getPositioninListPrincipal( string nameTeam ){
        //int index = unScoreTeams.getHighScoreTeamList().FindIndex(a => a.Contains(nameTeam));
        foreach ( HighScoreEntry _entry in highScoreEntryListPrincipal ){
            if( _entry.nameDepartamento.Equals(nameTeam) ){
                ///int index = myList.FindIndex(a => a.Contains("Tennis"));
                int index = highScoreEntryListPrincipal.FindIndex(a => a.Contains(_entry));
                return index;
            }
        }
        return -1;
    ...
    private class HighScoreEntry{
        ...
        public bool Contains( HighScoreEntry other ){
            if( other.nameDepartamento.Equals(nameDepartamento)){
                return true;
            }
            return false;
        }
