    ...
    */
    ...
    public void consultarScores(){
          ...
          if(storageService != null ){
              ...
              storageService.FindDocumentById(dbName,ManagerScoresJson.collectionNameHighScoresTeams,docId, new UnityCallBack());      
          } else {
    ...
    public List<Resultado> getListResultados(){
      ...
      public void OnSuccess(object response)  
            {  
                App42Log.Console("**** succes consulta high scores teams app42 storageService max "+max);  
                ...
                if( storage.GetJsonDocList() != null ){
                    ...
                    App42Log.Console("**** success consulta high scores teams app42 storageService count "+jsonDocList.Count);
                    int ultimo = jsonDocList.Count-1;
                    //for(int i=0;i <jsonDocList.Count;i++)  
                    //{     

                        App42Log.Console("objectId is " + jsonDocList[ultimo].GetDocId());  
                        string jsonDoc = String.Copy(jsonDocList[ultimo].GetJsonDoc());
                         ...
                         foreach (JSONNode objNode in jsonNode) {
                            ...
                            foreach (JSONNode jSONNode1 in objNode) {
                                ...
                                App42Log.Console(" iterando valores higt "); 
                                 int nivel = 1;
                                String nick = jSONNode1[NAME_NICK];
                                int score = jSONNode1[NAME_SCORE];
                                int turnos = jSONNode1[NAME_TURNOS];
                                int objetivo = 10;
                                String nameDevice = jSONNode1[NAME_DEVICE_NAME];
                                String nameModel = jSONNode1[NAME_DEVICE_MODEL ];
                                string fechaCreacion = jSONNode1[NAME_FECHA_CREACION ];
                                Resultado resultado = new Resultado(nick, nivel, objetivo, score, turnos, esPC, fechaCreacion );
                                lstResultados.Add(resultado);
                                //remplazarMejorScore(lstResultados,resultado);
