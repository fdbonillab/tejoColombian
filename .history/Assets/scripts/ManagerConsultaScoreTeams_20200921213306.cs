...
public class ManagerConsultaScoreTeams : App42CallBack
{
    ...
    */
    ...
    string docId = "5f69619ee4b083d8259e2fcb";
    ...
    public ScoreTeams getScoreTeams(){
        if( elScoreTeams != null ){
            App42Log.Console("**** pidiendo get score teams count "+elScoreTeams.getHighScoreTeamList().Count);
        }
        return elScoreTeams;
    }
    public string getIdDoc(){
      ...
      public void OnSuccess(object response)  
            {  
                App42Log.Console("**** success consulta high scores teams app42 storageService ");  
                Storage storage = (Storage) response; 
                IList<Storage.JSONDocument> jsonDocList = storage.GetJsonDocList();   
                for(int i=0;i <jsonDocList.Count;i++)  
                {     
                    idDoc = jsonDocList[i].GetDocId();
                    App42Log.Console(" teams objectId is " + idDoc );  
                    //App42Log.Console("jsonDoc is " + jsonDocList[i].GetJsonDoc());  
                    string jsonDoc = String.Copy( jsonDocList[i].GetJsonDoc());
                    App42Log.Console(" teams jsonDoc is " + jsonDoc );  
                    //JSONObject jsonObj = JsonUtility.FromJson<JSONObject>(jsonDoc);
                    //elScoreTeams = JsonUtility.FromJson<ScoreTeams>(jsonObj[Collision.NAME_HIGHSCORES]);
                    elScoreTeams = JsonUtility.FromJson<ScoreTeams>(jsonDoc);
                }    
            }  
